...
Drag nodes and edges to align the new polygon with template polygon. On the fill and stroke panel, under "Fill" tab, select "Flat color" and adjust the colour scales to label the new polygon. Then the first polygon is made successfully.   
...
Open the exported SVG image in Inkscape. Under "Object" tab at the top, select "Fill and Stroke...". First click the image, then select "Flat color" under "Fill" tab. Adjust colour scales to fill all shapes.   
...
All the paths in SVG image generated in GIMP are combined as a whole. In order to separate the paths, first click the image then select "Break Apart" under the "Path" tab on the top. Now the paths/shapes are separated, but the outlines of shapes are not stroked. Thus use "Ctrl+A" to select all polygons, on the fill and stroke panel select "Flat color" under "Stroke paint" tab, and set a number under "Stroke style" tab (e.g. 1.333 px).   
...
The same tissues in the SVG repository are already grouped. If the SVG image is made by following the above steps, same tissues should be grouped so as to be coloured in spatial heatmaps. Take the root_pGL2 as an example. Select polygons of the this sample by clicking polygon edge while pressing "Shift" key. Mouse over any edge of selected polygons, right click and select "Group".   
...
Under the "Edit" tab on the top, select "XML Editor...". Click the group to select it. On the "XML Editor (Shift+Ctrl+X)" panel, first click the id and type in "root_pGL2" then click "Set". It is critial that the id "root_pGL2" is identical with the corresponding tissue in the data matrix. After that, the first group is done.  
...
At last, it is required to group all groups and independent paths as a container group. To do so, use "Ctrl+A" to select all, mouse over the polygons, right click, and click group. It is optional to change the container group id. Note this step is already done in SVG images from the repository.   
