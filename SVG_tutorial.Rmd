...
To use the spatialHeatmap, the data matrix must be formatted in the following way:
...
The expression matrix is stored as an "SummarizedExperiment" object [@SummarizedExperiment].
...
```{r, filter, eval=TRUE, echo=TRUE, warnings=FALSE }
  ...
  # Filter genes. In "pOA", genes with expression value A >= 1 in at least p=0.03 (3%) of all samples are retained; in "CV", genes with coefficient of variance (cv) between 0.1 and 100 are retained. 
  exp <- filter_data(data=expr, pOA=c(1, 0.03), CV=c(0.1, 100), dir=NULL)
...
```{r, shm, eval=TRUE, echo=TRUE, warnings=FALSE, fig.align="center", fig.cap=('Figure 1. Spatial Heatmaps of the target gene \"PSAC\". User defined tissue regions are coloured by the expression profile of the target gene.')}
  ...
  spatial_hm(svg=svg.path, data=exp, pOA=c(0, 0), CV=c(0, 100), ID="PSAC", col.com=c("yellow", "blue", "purple"), width=1, height=1, sub.title.size=9.5, layout="gene", ncol=3)
...
## Troubleshooting
  ...
  3. Make sure all the paths, groups are placed in a large group/layer as a whole in the XML editor.
  4. Except for the large group/layer, make sure other groups contain only paths, no groups.
