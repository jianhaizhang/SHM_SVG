...
The gene expression matrix should be normalised and filtered before downstream processing. In this tutorial the normalisation process is not covered. The function `filter.data` [@SummarizedExperiment; @data.table; @utils] has two filter arguments `pOA` and `CV`, corresponding to `pOverA` and `cv` in the package "genefilter" [@Gentleman2018-xj], respectively.   
