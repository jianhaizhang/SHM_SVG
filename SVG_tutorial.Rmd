...
repository called [spatialHeatmap aSVG Repository](https://github.com/jianhaizhang/spatialHeatmap_aSVG_Repository), where some aSVG files developed in this project are already deposited (e.g. [Figure 1](#shm)). 
...
df <- subset(target.arab, sample=='root_pGL2' | sample=='root_pCO2')[, 2:3]
...
```{r eval=TRUE, echo=TRUE, warnings=FALSE }
se.aggr.arab <- aggr_rep(data=se.arab, sam.factor='sample', con.factor='condition', aggr='mean')
...
se.fil.arab <- filter_data(data=se.aggr.arab, sam.factor='sample', con.factor='condition', pOA=c(0.03, 6), CV=c(0.30, 100), dir=NULL)
```

# Simplified aSVG Format

There are two aSVG formats that `spatialHeatmap` accepts: simplified and [advanced](#advanced). The former is specifically desinged for `spatialHeatmap` while the latter is from [Expression Atlas Anatomogram](https://github.com/ebi-gene-expression-group/anatomogram/tree/master/src/svg). If the aSVGs are only used for making spatial heatmaps, the simplified format is recommended, since it is easier and time-saving.   
...
In order to be painted with the same color in spatial heatmaps, tissues consisting of multiple shapes should be grouped. Take the `root_pGL2` as example. Select shapes of this tissue by clicking their edges while pressing 'Shift' key. Mouse over any edge of selected shapes, right click and select 'Group'.   
...
A group (e.g. root_pSCR) could be resized by simply dragging the black arrows on the edge or changing the width (H), height (H) values on the top toolbar. On either case, in the "XML Editor" panel the resized group would have an attribute of "transform" with a value of "matrix", which means the coordinates of this group are transformed because of resizing.   
  ...
  The `spatialHeatmap` does not accept tranformed coordinates, and the tranform/matrix could be removed by ungrouping (Ctrl+Shift+G) and regrouping (Ctrl+G) the shapes.  
...
Save the aSVG file as 'arabidopsis_thaliana.root.cross_shm.svg' with the naming scheme '\<species\>[.view]_shm.svg', which is downloadable [here](https://github.com/jianhaizhang/SVG_tutorial_file/tree/master/download). Note: the aSVG file name ends with '.svg' and the front should only consist of letters, digits, dots, or underscores, and parenthesis should be avoided. E.g. 'arabidopsis_thaliana.root.cross_shm(1).svg' will throw out errors.   

By now the simplified sSVG file is done.  

# Spatial Heatmap {#shm}

The aSVG file 'arabidopsis_thaliana.root.cross_shm.svg' generated above is ready to use for plotting spatial heatmaps. It is packaged in `spatialHeatmap` and is accessed below.   

```{r eval=TRUE, echo=TRUE, warnings=FALSE}
svg.root <- system.file('extdata/shinyApp/example/arabidopsis_thaliana.root.cross_shm.svg', package='spatialHeatmap')
```
...
spatial_hm(svg.path=svg.root, data=se.fil.arab, sam.factor='sample', con.factor='condition', ID=c("HRE2"), height=0.4, legend.nrow=4, legend.r=1.5, legend.width=0.9)
```

# Supplement

This section presents details of data formatting on `vector` and `data frame` and the advanced aSVG format.   
...
In the downstream interactive network (refer to the [spatialHeatmap vignette](https://jianhaizhang.github.io/SVG_tutorial_file/spatialHeatmap.html)), if users want to have a gene annotation by mousing over a node, a column of gene annotation can be appended to the data frame. For example, the 20 genes are annotated as ann1, ann2, ..., ann20.   
...
## Advanced aSVG Format {#advanced}

The advanced aSVG format is compatible with both spatialHeatmap and [Expression Atlas Anatomogram](https://github.com/ebi-gene-expression-group/anatomogram/tree/master/src/svg), and the specific requirements are listed at the bottom of this [page](https://github.com/ebi-gene-expression-group/anatomogram/tree/master/src/svg). The following sections illustrate the transition from the simplified format to advanced format.   
...
The EBI SVG follows the naming scheme '\<species\>[.view].svg', so this image can be saved as 'arabidopsis_thaliana.root.cross.svg', which is downloadable [here](https://raw.githubusercontent.com/jianhaizhang/SVG_tutorial_file/master/download/arabidopsis_thaliana.root_cross.svg) and ready to use in section [Spatial Heatmap](#shm).  
